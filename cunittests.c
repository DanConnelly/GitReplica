...
void simpler_sample_test(void)
{
    // This is even more basic. This tests if beargit_status will
    // do something. Then check if the TEST_STDOUT file matches some lines.
  int check;
  check = beargit_init();
  CU_ASSERT(0==check);
  check = beargit_add("asdf.txt");
  CU_ASSERT(0==check);
  check = beargit_status();
  CU_ASSERT(0==check);
  ...
  while(fgets(line, sizeof(line),fstdout)) {
    if (count == 0) {
          CU_ASSERT(!strncmp(line,"Tracked files:\n", strlen("Tracked files:\n")));
    } else if (count == 1) {
          CU_ASSERT(!strncmp(line,"\n", strlen("\n")));
    } else if (count == 2) {
          CU_ASSERT(!strncmp(line,"asdf.txt\n", strlen("asdf.txt\n")));
    } else if (count == 3) {
          CU_ASSERT(!strncmp(line,"\n", strlen("\n")));
    } else if (count == 4) {
...
int cunittester()
{
   ...
   // Adds a suite 3 to the registry
   pSuite3 = CU_add_suite("Suite_N", init_suite, clean_suite);
   if (NULL == pSuite3) {
    CU_cleanup_registry();
    return CU_get_error();
   }
   // Adds a test to Suite #3
   if (NULL == CU_add_test(pSuite3, "Simpler Test for Beargit status", simpler_sample_test))
   {
    CU_cleanup_registry();
    return CU_get_error();
   }
